...
listCreate ()
{
   List l = (List) malloc (sizeof (List));//aloca memoria
   if (l == NULL)//confere se foi devidamente alocado
      return NULL;

   l->p = l->u = NULL;//instancia o ponteiro para o primeiro e ultimo elemento como null
   l->n = 0;//numero de itens na lista 0
...
listDestroy (List l)
{
   ...
   if ( l == NULL) return;//caso nao seja passado um ponteiro valido

   c = l->p;//pega o ponteiro para o inicio da lista
   while (c != NULL)
   {
      x = c->next;
      free (c);
...
listGetI (List l, int p)
{
   ...
   for (j = 1; j < p; j++, c = c->next)//enquanto nao chegar no indice p da lista
	 ;

   return c->i;//retorna o ponteiro para o item no indice p
}
...
listIns (ItemList i, int p, List l)
{
   ...
   x->i = (ItemList)malloc(sizeof(i));//aloca memoria para o item
   memcpy(x->i , i, sizeof(i));//copia o item para celula x da lista
